...
if select == "Home":
    ...
    with tab1:
        ...
        #     pass
            ...
            # df_user = db.df_aggregated_user.rename(
            #     columns={"count": "user_count", "percentage": "user_percentage"}
            # )

            aguy_agg_user_Y = Agg_user_plot_1(db.df_aggregated_user, years)

            if not aguy_agg_user_Y.empty and "state" in aguy_agg_user_Y.columns:
                col1, col2 = st.columns(2)
                with col1:
                    stat = st.selectbox("Select the state",
                                        sorted(aguy_agg_user_Y["state"].unique()), key="state_by_year")
                aguy_agg_user_Y_state = Agg_user_plot_3(aguy_agg_user_Y, years, stat)

            if not aguy_agg_user_Y.empty and "quarter" in aguy_agg_user_Y.columns:
                col1, col2 = st.columns(2)
                with col1:
                    quarters = st.slider("Select the quarter",
                                        int(aguy_agg_user_Y["quarter"].min()),
                                        int(aguy_agg_user_Y["quarter"].max()))
                aguy_agg_user_Y_Q = Agg_user_plot_2(aguy_agg_user_Y, years, quarters)

                if not aguy_agg_user_Y_Q.empty and "state" in aguy_agg_user_Y_Q.columns:
                    col1, col2 = st.columns(2)
                    with col1:
                        stat2 = st.selectbox("Select the state",
                                            sorted(aguy_agg_user_Y_Q["state"].unique()), key="state_by_quarter")
                    aguy_agg_user_Y_state_Q = Agg_user_plot_4(
                        aguy_agg_user_Y_Q, years, quarters, stat2)
            else:
                st.warning("Quarter data not available for this year.")
